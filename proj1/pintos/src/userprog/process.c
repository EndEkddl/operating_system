...
process_execute (const char *file_name) 
{
  ...
  for(i=0; i<strlen(file_name); i++){
	...
	progName[i] = file_name[i];
  }
  progName[i] = '\0';

  file_ds = filesys_open(progName);

  if(file_ds == NULL) return -1;
...
process_wait (tid_t child_tid UNUSED) 
{
	...
	if(child_tid < 0 || parent == NULL) return -1;
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  //make up the stack(Argument Passing)

  for(i=argc-1; i>=0; i--){
	int arguLen = strlen(argv[i])+1;
	(*esp) -= arguLen;
	strlcpy((*esp), argv[i], arguLen);
	argv[i] = (char*)(*esp);
  }
