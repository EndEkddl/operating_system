...
struct bitmap* bitmap_expand(struct bitmap *bitmap, int size){
	...
	struct bitmap *expanded;
	size_t prevSize = bitmap_size(bitmap);

	expanded = bitmap_create(prevSize + size);

	if(expanded == NULL) printf("bitmap_expand is error!\n");
	else{
		for(int i=0; i<prevSize; i++){
			if(bitmap_test(bitmap, i)) bitmap_mark(expanded, i);
		}
	}
